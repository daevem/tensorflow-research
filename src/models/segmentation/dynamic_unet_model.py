...
class DynamicUnetModel(BaseModel):
    ...
    def compile(self, loss="categorical_crossentropy"):
        self.model.compile(
            ...
            metrics=["accuracy", MeanIoU(num_classes=4, name="MeanIoU_noBG", one_hot=True)]  # , mean_iou_no_bg],
        )
...
def conv2d_block(
    ...
    # TODO consider restyling conv+relu+bn to conv+bn+relu
    c = Conv2D(
        filters,
        kernel_size,
...
class MeanIoU(tf.keras.metrics.Metric):
    ...
    def __init__(self, num_classes, name=None, dtype=None, one_hot=False):
